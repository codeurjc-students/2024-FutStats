...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    List<Match> findAllByLeague(@Param("league") String league);
    ...
    @Query("SELECT (SUM(CASE WHEN p.team1.id = :id THEN p.possesion1 ELSE 0 END) + SUM(CASE WHEN p.team2.id = :id THEN p.possesion2 ELSE 0 END)) /  + COUNT(p) FROM Match p WHERE p.team1.id = :id OR p.team2.id = :id")
    float findAveragePossessionByTeam(@Param("id") Long id);
